...
So, I decided to create my own version of Content Collections. I created a `content` folder, and inside separated the content by locale. I also created a `content/index.ts` file that contains helpers for parsing the frontmatter using [valibot](https://valibot.dev/) and getting both the list of posts and the post itself for a given slug and locale.
...
```ts
// Posts are stored in the `content` folder, we can use `import.meta.glob` to lazy load all of the MDX files, essentially create a dictionary with the post slugs as keys and the functions that load the post as values.
const BLOG_POST_LIST = import.meta.glob("/src/content/**/*.mdx");

// Using server$ to make sure this code only runs on the server
export const getPostBySlug = server$(async (slug: string, locale: string) => {
  ...
  // This should have everything we need to render the post, since Vite will have already compiled it for us when we import it.
  const resource = await importPost();

  // Send only the relevant data to the client
  const post = {
    slug,
...
export const getPostsByLocale = server$(async (locale: string) => {
  ...
  const postsByLocale = await Promise.all(
    paths.map(async (path) => {
      // Same as before, we can just import the post and get the frontmatter
      const resource = await BLOG_POST_LIST[path]();
      const frontmatter = parse(FRONTMATTER_SCHEMA, resource.frontmatter);
      // We can get the slug from the path, since the file name and the slug are the same
...
And then we can load it using `routerLoader$` and send to your page to render the post.

```tsx
export const usePost = routeLoader$(async ({ params, error }) => {
  // We can use the created @angular/localize helpers to get the current locale
