...
/**
 * Replaces the dev image src with the production imgix src.
 *
 * @param src - The image src
 * @returns The image src with imgix URL in production
 */
export const getThumbnailSource = (src: string) => {
  if (isDev) return src;

  const parsedSrc = src.replace("/assets/", "");
  return `${import.meta.env.PUBLIC_IMGIX_URL}/${parsedSrc}`;
};
